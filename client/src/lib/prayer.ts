...
export async function createPrayerRequest({
  ...
  tags = [],
  is_anonymous = false,
}: PrayerRequestCreateParams): Promise<ApiResponse<any>> {
  try {
...
export async function listPrayerRequests({
  ...
  try {
    console.log(
      "🙏 [listPrayerRequests] Starting prayer requests fetch via API...",
      { status, limit },
    );
...
export async function getPrayerRequest(id: number): Promise<ApiResponse<any>> {
  try {
    ...
    if (!response.ok) {
      console.error(
        "Failed to fetch prayer request:",
        response.status,
        response.statusText,
      );
 ...
 */
export async function commitToPray(
  requestId: number,
  ...
  try {
    ...
    const spamCheck = await checkPrayerCommitmentSpam(
    ...
    if (!spamCheck.allowed) {
      return {
        data: null,
        error: spamCheck.reason || "Unable to commit at this time",
      };
    }

    const baseUrl = getApiBaseUrl();
    const response = await fetch(
      `${baseUrl}/api/prayer-requests/${requestId}/commit`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
    ...
    if (!response.ok) {
      ...
      return {
        data: null,
        error: errorData.error || "Failed to commit to prayer",
      };
    }
    ...
    return {
      data: {
        ...data,
        spamWarning:
          spamCheck.warningLevel !== "none" ? spamCheck.reason : null,
      },
      error: null,
    };
  } catch (err) {
 ...
 */
export async function uncommitToPray(
  requestId: number,
): Promise<ApiResponse<any>> {
  try {
    const { data: user } = await supabase.auth.getUser();
    ...
    // Log activity
    await supabase.from("prayer_activity").insert({
 ...
 */
export async function confirmPrayed(
  requestId: number,
  ...
  try {
    ...
    const baseUrl = getApiBaseUrl();
    const response = await fetch(
      `${baseUrl}/api/prayer-requests/${requestId}/confirm-prayed`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
    ...
    if (!response.ok) {
      ...
      return {
        data: null,
        error: errorData.error || "Failed to confirm prayer",
      };
    }
    ...
    try {
      ...
      if (
        prayerRequest?.requester &&
        prayerRequest.requester !== sessionData.session.user.id
      ) {
        createNotification({
...
export async function getMyCommitments({
  status,
  limit = 20,
}: PrayerCommitmentParams = {}): Promise<ApiResponse<any[]>> {
  try {
    const { data: sessionData } = await supabase.auth.getSession();
...
export async function getMyRequests({
  ...
  try {
    ...
    const enhancedData = data.map((request) => ({
      ...
      prayer_stats: {
        committed_count:
          request.prayer_commitments?.filter((c) => c.status === "committed")
            .length || 0,
        prayed_count:
          request.prayer_commitments?.filter((c) => c.status === "prayed")
            .length || 0,
